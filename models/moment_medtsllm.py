...
class MomentMedTsLLM(MedTsLLM):
    ...
    def __init__(self, config, dataset) -> None:
        ...
        self.lead_aggregation = str(
            self.moment_cfg.get("lead_aggregation", "attention")
        ).lower()

        if self.lead_aggregation not in {"attention", "mean"}:
            raise ValueError(
                "models.medtsllm.moment.lead_aggregation must be "
                "'attention' or 'mean', got "
                f"{self.lead_aggregation!r}"
            )

        self.use_highres_windows = bool(
            self.moment_cfg.get("use_highres_windows", True)
        )
        self.save_frozen_backbone = bool(
            self.moment_cfg.get("save_frozen_backbone", False)
        )
    ...
    def _aggregate_leads(
        ...
        """Aggregate MOMENT representations across ECG leads.
        ...
        attention:
            Uses the proposed learned lead-attention module.

        mean:
            Uniformly averages the raw MOMENT lead representations.
            No learned lead embedding or scoring network is used.
        """
        if x.ndim != 4:
            raise ValueError(
                "_aggregate_leads expects [B,C,N,D], "
                f"received {tuple(x.shape)}"
            )

        if self.lead_aggregation == "attention":
            return self.lead_attention(x)

        # Mean-lead ablation.
        tokens = x.mean(dim=1)

        # Return uniform weights so the diagnostics interface stays
        # compatible with the attention version.
        batch, leads, patches, _ = x.shape

        weights = torch.full(
            (batch, leads, patches),
            fill_value=1.0 / float(leads),
            device=x.device,
            dtype=x.dtype,
        )
    ...
    def _encode_moment(
        ...
        aligned_tokens, aligned_lead_weights = self._aggregate_leads(
            aligned_raw
        )
        ...
        if window_count > 0:
            ...
            window_tokens, _ = self._aggregate_leads(
                window_raw
            )
